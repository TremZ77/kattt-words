...
export class SuperScrabbleGame {
    ...
    addPlayer(id: string, name: string, userId: string = '', isBot: boolean = false) {
        // If it's a real user (has userId), check if they are already in the game
        if (userId && !isBot) {
            const existingPlayer = this.state.players.find(p => p.userId === userId);
            if (existingPlayer) {
                existingPlayer.id = id; // Update to new socket ID
                existingPlayer.name = name; // Update name just in case
                // Migration: Initialize moveHistory if it doesn't exist
                if (!existingPlayer.moveHistory) {
                    existingPlayer.moveHistory = [];
                }
                return true;
            }
        }
