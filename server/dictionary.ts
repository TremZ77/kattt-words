...
const getDictionaryPath = () => {
    ...
    const paths = [
        path.join(__dirname, 'data', 'dictionary.txt'),      // Dev (ts-node) or if copied relative
        path.join(__dirname, '..', 'data', 'dictionary.txt'), // Prod (if in dist/server/index.js -> dist/data)
        path.join(__dirname, '..', '..', 'data', 'dictionary.txt'), // Prod (if in dist/index.js -> root/data)
        path.join(process.cwd(), 'server', 'data', 'dictionary.txt'), // Fallback
        path.join(process.cwd(), 'data', 'dictionary.txt') // Fallback
    ];
