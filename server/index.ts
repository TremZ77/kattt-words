...
io.on('connection', (socket) => {
    ...
    socket.on('getRooms', () => {
        // Return list of public lobbies (LOBBY status)
        const roomList = Object.values(games)
            .filter(g => g.getState().gameStatus === 'LOBBY')
            .map(g => ({
                id: g.getState().roomCode,
                players: g.getState().players.length
            }));
        socket.emit('roomList', roomList); // Need to add to types
    });
    ...
    socket.on('startGame', () => {
        ...
        if (roomCode) {
            const game = games[roomCode];
            if (game) {
                // Only host (first player?) or any ready player?
                // Ideally Host. But for simplicity, any player if all ready.
                // We trust the client to show the button only to host.
                game.startGame();
                io.to(roomCode).emit('gameUpdate', game.getState());
            }
        }
