            ...
            for (const p of perms) {
                ...
                if (isFirstMove) {
                    ...
                    for (const anchor of shuffledOccupied) {
                        ...
                        const tryMove = (move: { r: number, c: number, tile: Tile }[]) => {
                            // 1. Validate Geometry
                            if (!validateMove(board, move, false).isValid) return null;

                            // 2. Validate All Words (including cross words)
                            const { words } = calculateMoveScore(board, move);
                            const allValid = words.every(w => isValidWord(w.word));

                            if (allValid) return move;
                            return null;
                        };
                        ...
                        if (anchor.c + 1 < BOARD_SIZE) {
                            ...
                            const validMove = tryMove(moveRight);
                        ...
                        if (anchor.c - 1 >= 0) {
                            ...
                            if (startC >= 0) {
                                ...
                                const validMove = tryMove(moveLeft);
                                if (validMove) return validMove;
                            }
                        }
                        ...
                        if (anchor.r + 1 < BOARD_SIZE) {
                            ...
                            const validMove = tryMove(moveDown);
                            if (validMove) return validMove;
                        }
                        ...
                        if (anchor.r - 1 >= 0) {
                            ...
                            const validMove = tryMove(moveUp);
                            if (validMove) return validMove;
                        }
                    }
                }
